...
class DokumentModel(QtCore.QObject):
    ...
    def set_tockeUmjeravanja(self, x, recalculate=True):
        ...
        if x != self.tockeUmjeravanja:
            ...
            output=[self.tockeUmjeravanja, self.siroviPodaciStart, recalculate]
            self.emit(QtCore.SIGNAL('promjena_tockeUmjeravanja(PyQt_PyObject)'),
                      output)
    ...
    def set_cNOx50(self, x, recalculate=True):
        ...
        if x != self.cNOx50:
            self.cNOx50 = x
            output = [self.cNOx50, recalculate]
    ...
    def set_cNOx95(self, x, recalculate=True):
        ...
        if x != self.cNOx95:
            self.cNOx95 = x
            output = [self.cNOx95, recalculate]
    ...
    def pomakni_tocke_umjeravanja(self, x, recalculate=True):
        """
        Pomicanje tocaka umjeravanja ovisno o pocetku umjeravanja.
        x je novi indeks za pocetak umjeravanja (integer).
        """
        #pomakni tocke
        delta = x - self.siroviPodaciStart
        for tocka in self.tockeUmjeravanja:
            value = set([ind + delta for ind in list(tocka.indeksi)])
            tocka.indeksi = value
        #zapamti novi start
        self.siroviPodaciStart = x
        #promjeni pocetak i kraj umjeravanja (membere)
        self.set_vremenske_granice_umjernih_tocaka()
        #emitiraj promjenu tocaka i starta
        output=[self.tockeUmjeravanja, self.siroviPodaciStart, recalculate]
        self.emit(QtCore.SIGNAL('update_tablicu_sirovihPodataka_umjeravanje(PyQt_PyObject)'),
                  output)

    def pomakni_tocke_provjere_konvertera(self, x, recalculate=True):
        """
        Pomicanje tocaka provjere konvertera ovisno o pocetku umjeravanja.
        x je novi indeks za pocetak provjere konvertera (integer).
        """
        #pomakni tocke
        delta = x - self.konverterPodaciStart
        for tocka in self.tockeKonverter:
            value = set([ind + delta for ind in list(tocka.indeksi)])
            tocka.indeksi = value
        #zapamti novi start
        self.konverterPodaciStart = x
        #emitiraj promjenu tocaka i start
        output = [self.tockeKonverter, self.konverterPodaciStart, recalculate]
        self.emit(QtCore.SIGNAL('update_tablicu_sirovihPodataka_konverter(PyQt_PyObject)'),
                  output)


    def set_siroviPodaciStart(self, x, shift=True, recalculate=True):
        ...
        if isinstance(x, QtCore.QModelIndex):
            x = x.row()
        x = int(x)
        if x != self.siroviPodaciStart or x == 0:
            if shift:
                self.pomakni_tocke_umjeravanja(x, recalculate=recalculate)
            else:
                self.siroviPodaciStart = x
    ...
    def set_konverterPodaciStart(self, x, shift=True, recalculate=True):
        ...
        if isinstance(x, QtCore.QModelIndex):
            x = x.row()
        x = int(x)
        if x != self.konverterPodaciStart or x == 0:
            if shift:
                self.pomakni_tocke_provjere_konvertera(x, recalculate=recalculate)
            else:
                self.konverterPodaciStart = x
    ...
    def dict_to_dokument(self, mapa):
        ...
        self.set_tockeUmjeravanja(umjerneTocke, recalculate=False)
